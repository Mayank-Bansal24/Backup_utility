...
class file_data {
    ...
    file_data           (fs::path loc);             /* Parameterized constructor */
    file_data           (json obj, int x);             /* Constructor that loads data from json */
    
    json                dump_file_data ();      /* Returns the object as json */
    intmax_t            get_file_size ();       /* Returns the file size */
    fs::path            get_path ();            /* Returns the path of the file */
